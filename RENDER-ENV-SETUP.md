...
| `GEOCODE` | `0` (only if you want it off) | Optional. Maps resolve any place on earth by looking up unknown names via OpenStreetMap, cached permanently. Leave unset to keep this on. |
| `EXCHANGERATE_API_KEY` | your exchangerate-api.com key | Optional. Enables the "Show in" currency dropdown on Workforce Time & Cost so costs convert to any world currency. Without it costs show in each client's own currency. |
...
## If the custom domain shows "Not Found"

A bare **Not Found** page (Render's own page, not the app) means the request reached Render
but no service claims that hostname. Adding the DNS record alone is not enough:

1. Render dashboard → your service → **Settings → Custom Domains → Add Custom Domain**
2. Enter `analytics.statspack.africa` exactly
3. Render shows the DNS target it expects; make sure the CNAME at name.com matches it
4. Wait for Render to show **Verified** and issue the certificate

Until step 2 is done, the `.onrender.com` URL works and the custom domain returns Not Found.
