...
credentials are printed only in the server console/logs** (on Render: your service's *Logs* tab) —
...
## Stable API keys without a disk

If you run without a persistent disk (e.g. Render Free), the database re-seeds on every
restart, which would normally regenerate the seeded clients' API keys. To keep them stable,
set environment variables in Render (**Environment** tab):

* `TEST_API_KEY` — pins the "VMS Test Environment" client's key (recommend `spk_test_` + 32 hex chars)
* `DEMO_API_KEY` — optionally pins the demo client's key

The seeded logins and these keys then survive every restart. Note that *visit data* still
resets without a disk — only the Starter plan + disk preserves data.
...
1. Open the URL → sign in with the initial super-user credentials from the server logs.
2. **Immediately change the seeded passwords** under Account → Security, and create your real
   client + admin logins in the Client Console.
